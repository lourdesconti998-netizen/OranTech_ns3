...
#include "ns3/oran-lm-nr-rl-qlearn.h"
...
NS_LOG_COMPONENT_DEFINE("OranNrExampleRlQlearn");
...
main(int argc, char* argv[])
{
    ...
    nrHelper->SetSchedulerTypeId(ns3::NrMacSchedulerOfdmaUlTarget::GetTypeId());
    
    Ptr<Node> pgw = epcHelper->GetPgwNode();

    // BWP (Bandwidth Part) para una portadora (CC Component Carrier)
    ...
    // LM + CMM
    Ptr<OranLm> defaultLm = CreateObject<OranLmNrRlQlearn>();
    ...
    Ptr<OranNrNearRtRicE2Terminator> nearRtRicE2Terminator = CreateObject<OranNrNearRtRicE2Terminator>();
