...
NS_LOG_COMPONENT_DEFINE("OranLmNrRlDqn");
NS_OBJECT_ENSURE_REGISTERED(OranLmNrRlDqn);

TypeId
OranLmNrRlDqn::GetTypeId()
{
  static TypeId tid = TypeId("ns3::OranLmNrRlDqn").SetParent<OranLm>().AddConstructor<OranLmNrRlDqn>();
  return tid;
}

OranLmNrRlDqn::OranLmNrRlDqn()
{
  m_name = "OranLmNrRlDqn";
...
OranLmNrRlDqn::~OranLmNrRlDqn() = default;
...
if (m_grpc)
{
  ...
  if (status.ok())
  {
    ...
    if (!gnbIds.empty() && response.ue_shares_size() > 0)
    {
      ...
      for (const auto& kv : ulTargets)
      {
        NS_LOG_UNCOND("  rnti=" << kv.first << " target=" << kv.second);
      }
    }
  }
  else
...
uint64_t
OranLmNrRlDqn::GetBufferSize(Ptr<OranNrDataRepository> data, uint64_t ueId, uint8_t lcid)
{
...
uint64_t
OranLmNrRlDqn::GetTxDrop(Ptr<OranNrDataRepository> data, uint64_t ueId, uint8_t lcid)
{
 // Se incrementa contador de llamadas.
...
// El valor se actualiza cada m_N llamadas y se mantiene el último valor entre consultas.
uint64_t
OranLmNrRlDqn::GetTxPdu(Ptr<OranNrDataRepository> data, uint64_t ueId, uint8_t lcid)
{
 // Se incrementa contador de llamadas.
    m_txPduCounter[ueId]++;
