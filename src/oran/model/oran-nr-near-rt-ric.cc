...
NS_LOG_COMPONENT_DEFINE("OranNrNearRtRic");

NS_OBJECT_ENSURE_REGISTERED(OranNrNearRtRic);

TypeId
OranNrNearRtRic::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::OranNrNearRtRic")
            .SetParent<Object>()
            .AddConstructor<OranNrNearRtRic>()
            .AddAttribute("DefaultLogicModule",
                          "The default logic module.",
                          PointerValue(nullptr),
                          MakePointerAccessor(&OranNrNearRtRic::m_defaultLm),
                          MakePointerChecker<OranLm>())
            .AddAttribute("E2Terminator",
                          "The E2 Terminator for the Near-RT RIC.",
                          PointerValue(nullptr),
                          MakePointerAccessor(&OranNrNearRtRic::m_e2Terminator),
                          MakePointerChecker<OranNrNearRtRicE2Terminator>())
            .AddAttribute("DataRepository",
                          "The data repository.",
                          PointerValue(nullptr),
                          MakePointerAccessor(&OranNrNearRtRic::m_data),
                          MakePointerChecker<OranNrDataRepository>())
            .AddAttribute("ConflictMitigationModule",
                          "The Conflict Mitigation Module.",
                          PointerValue(nullptr),
                          MakePointerAccessor(&OranNrNearRtRic::m_cmm),
                          MakePointerChecker<OranCmm>())
            .AddAttribute("LmQueryInterval",
                          "Interval between periodic queries to the Logic Modules",
                          TimeValue(Seconds(5)),
                          MakeTimeAccessor(&OranNrNearRtRic::m_lmQueryInterval),
            ...
            .AddAttribute("LmQueryMaxWaitTime",
                          ...
                          "indicates no limit.",
                          TimeValue(Seconds(0)),
                          MakeTimeAccessor(&OranNrNearRtRic::m_lmQueryMaxWaitTime),
                          MakeTimeChecker())
            .AddAttribute(
                "LmQueryLateCommandPolicy",
                "The filter to apply on UL CQIs received",
                EnumValue(OranNrNearRtRic::DROP),
                MakeEnumAccessor<LateCommandPolicy>(&OranNrNearRtRic::m_lmQueryLateCommandPolicy),
                MakeEnumChecker(OranNrNearRtRic::DROP, "DROP", OranNrNearRtRic::SAVE, "SAVE"))
            .AddAttribute("E2NodeInactivityThreshold",
                          "The amount of time from a node's last registration request before "
                          "becoming inactive.",
                          TimeValue(Seconds(5)),
                          MakeTimeAccessor(&OranNrNearRtRic::m_e2NodeInactivityThreshold),
                          MakeTimeChecker())
            .AddAttribute(
                ...
                MakePointerAccessor(&OranNrNearRtRic::m_e2NodeInactivityIntervalRv),
                MakePointerChecker<RandomVariableStream>());

    return tid;
}

OranNrNearRtRic::OranNrNearRtRic()
    : Object(),
...
bool
OranNrNearRtRic::IsActive() const
{
    NS_LOG_FUNCTION(this);
...
OranNrNearRtRic::Start()
{
    ...
    m_lmQueryEvent = Simulator::Schedule(m_lmQueryInterval, &OranNrNearRtRic::QueryLms, this);
    m_e2NodeInactivityEvent = Simulator::Schedule(Seconds(m_e2NodeInactivityIntervalRv->GetValue()),
                                                  &OranNrNearRtRic::CheckForInactivity,
                                                  this);
}

void
OranNrNearRtRic::Stop()
{
    NS_LOG_FUNCTION(this);
...
Ptr<OranNrNearRtRicE2Terminator>
OranNrNearRtRic::GetE2Terminator() const
{
    NS_LOG_FUNCTION(this);
...
Ptr<OranLm>
OranNrNearRtRic::GetDefaultLogicModule() const
{
    NS_LOG_FUNCTION(this);
...
void
OranNrNearRtRic::SetDefaultLogicModule(Ptr<OranLm> newDefaultLm)
{
...
Ptr<OranLm>
OranNrNearRtRic::GetAdditionalLogicModule(std::string name) const
{
...
OranNrNearRtRic::AddLmResult
OranNrNearRtRic::AddLogicModule(Ptr<OranLm> newLm)
{
    NS_LOG_FUNCTION(this);

    AddLmResult ret = OranNrNearRtRic::ADDLM_OK;
    ...
    else
    {
        ...
        ret = OranNrNearRtRic::ADDLM_ERR_NAME_EXISTS;
    }

    return ret;
}

OranNrNearRtRic::RemoveLmResult
OranNrNearRtRic::RemoveLogicModule(std::string name)
{
    NS_LOG_FUNCTION(this);

    RemoveLmResult ret = OranNrNearRtRic::DELLM_OK;
    ...
    else
    {
        ...
        ret = OranNrNearRtRic::DELLM_ERR_NAME_INVALID;
    }
...
bool
OranNrNearRtRic::AddQueryTrigger(std::string name, Ptr<OranQueryTrigger> trigger)
{
...
bool
OranNrNearRtRic::RemoveQueryTrigger(std::string name)
{
    NS_LOG_FUNCTION(this << name);
...
Ptr<OranNrDataRepository>
OranNrNearRtRic::Data() const
{
    NS_LOG_FUNCTION(this);
...
void
OranNrNearRtRic::SetCmm(Ptr<OranCmm> newCmm)
{
...
void
OranNrNearRtRic::NotifyLmFinished(Time cycle, std::vector<Ptr<OranCommand>> commands, Ptr<OranLm> lm)
{
...
void
OranNrNearRtRic::NotifyReportReceived(Ptr<OranReport> report)
{
        ...
        if (m_lmQueryMaxWaitTime > Seconds(0))
        {
            m_processLmQueryCommandsEvent =
                Simulator::Schedule(m_lmQueryMaxWaitTime,
                                    &OranNrNearRtRic::ProcessLmQueryCommands,
                                    this);
        }
