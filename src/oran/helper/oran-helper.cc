...
#include "ns3/oran-nr-data-repository.h"
#include "ns3/oran-e2-node-terminator.h"
#include "ns3/oran-lm.h"
#include "ns3/oran-nr-near-rt-ric-e2terminator.h"
    ...
    static TypeId tid =
        TypeId("ns3::OranHelper")
            ...
            .AddAttribute("LmQueryLateCommandPolicy",
                          ...
                          MakeEnumAccessor<OranNrNearRtRic::LateCommandPolicy>(
      ...
      m_reporterFactories(std::vector<ObjectFactory>())
{
    ...
    m_dataRepositoryFactory.SetTypeId("ns3::OranNrDataRepositorySqlite");
...
OranHelper::CreateNearRtRic()
{
    ...
    Ptr<OranNrDataRepository> dataRepository = m_dataRepositoryFactory.Create<OranNrDataRepository>();
    ...
    for (auto lmFactory : m_lmFactories)
    {
        ...
        if (nearRtRic->AddLogicModule(lm) == OranNrNearRtRic::ADDLM_OK)
        {
...
void
OranHelper::ActivateAndStartNearRtRic(Ptr<OranNrNearRtRic> nearRtRic)
{
    nearRtRic->Start();
}

void
OranHelper::DeactivateAndStopNearRtRic(Ptr<OranNrNearRtRic> nearRtRic)
{
    nearRtRic->Stop();
}

OranE2NodeTerminatorContainer
OranHelper::DeployTerminators(Ptr<OranNrNearRtRic> nearRtRic,
                              NodeContainer nodes,
                              uint32_t netDeviceIndex)
{
    ...
    for (auto nodeIt = nodes.Begin(); nodeIt != nodes.End(); nodeIt++)
    {
        Ptr<OranNrE2NodeTerminator> terminator =
            m_e2NodeTerminatorFactory.Create<OranNrE2NodeTerminator>();
